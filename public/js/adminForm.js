...
(function () {
  ...
  async function loadClientStrict() {
    try {
      // Note: We use the API_BASE established in the previous closure via window config or hardcoded
      const res = await fetch(`https://smartcardlink-api.onrender.com/api/clients/${clientId}`);
      const json = await res.json();
      ...
      // Populate theme picker specifically
      const picker = document.getElementById('themeColorPicker');
      const textInput = document.getElementById('themeColor');
      if (client.themeColor) {
        if (picker) picker.value = client.themeColor;
        if (textInput) textInput.value = client.themeColor;
      }
...
// Sync color picker with text input (Global scope within module)
(function() {
    const picker = document.getElementById('themeColorPicker');
    const textInput = document.getElementById('themeColor');
    if(picker && textInput) {
