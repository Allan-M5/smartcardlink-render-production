...
            // CRITICAL FIX: Default to 'Pending' status if no filter is selected
            const effectiveStatus = selectedStatus || 'Pending';
            
            // Use the Admin API route with query params
            const response = await fetch(`${API_ROOT}/admin/clients?q=${encodeURIComponent(searchTerm)}&status=${encodeURIComponent(effectiveStatus)}`);
            
            if (!response.ok) {
...
            // assuming standardized API: { status, data, meta }
            allClientData = Array.isArray(json) ? json : (json.data || []);
...
// CRITICAL FIX FOR PROBLEM 6.2: Make function async and add status update before redirect.
async function processClient(clientId, notes = 'No reason provided') {
    // 1. Update status to 'Processed' 
    // This will automatically refresh the dashboard via fetchAllClients inside changeClientStatus.
    try {
        await changeClientStatus(clientId, 'Processed', notes); 
    } catch (e) {
        // If status update fails, still proceed to open form, but log a warning.
        console.error("Warning: Failed to set status to 'Processed' via API. Opening form anyway.", e);
        showToast("Warning: Status update failed. Opening Admin Form. Check server logs.", 'error');
    }

    // 2. Open Admin Form for actual processing (using current window.open('_blank') method)
    const url = new URL(ADMIN_FORM_URL, window.location.href);
    url.searchParams.set('id', clientId);
    
    // Using window.open('_blank') to keep the dashboard open for reference (best practice for admin flow).
...
    // Ensure default filter is 'Pending' on first load
