...
const ClientSchema = new mongoose.Schema({
  ...
  // Professional Customization
    ...
    // Status and Media
    photoUrl: { type: String, default: "" },`n    themeColor: { type: String, default: "#FFD700" },`n    themeName: { type: String, default: "modern" }, // Cloudinary URL
...
mongoose
    .connect(MONGO_URI)
    .then(() => logger.info("ц?е⌠Б? MongoDB connected successfully"))
    .catch((err) => {
        logger.error({ err }, "ц?ще▓ MongoDB connection error. Check MONGODB_URI.");
        process.exit(1);
    });
...
const sendEmail = async (to, subject, text, html) => {
    ...
    try {
        ...
        logger.error({ err }, `ц?ще▓ Failed to send email to ${to}`);
    }
};
    ...
    helmet.contentSecurityPolicy({
        directives: {
            ...
            connectSrc: [
                ...
                new URL(VCARD_BASE_URL).origin, ц┌? // Added for public vCard access
                "res.cloudinary.com", 
...
app.post("/api/upload-photo", publicLimiter, upload.single("photo"), async (req, res) => {
    try {
        ...
        logger.error({ err }, "ц?ще▓ POST /api/upload-photo error");
...
app.post("/api/clients", publicLimiter, validateClientCreation, handleValidationErrors, async (req, res) => {
    try {
        ...
        if (ADMIN_EMAIL) {
            ...
            const text = `New client submitted. ID: ${clientDoc._id} ц?Б┌?Б? ${clientDoc.fullName}. Check admin panel to process.`;
        ...
        logger.error({ err }, "ц?ще▓ POST /api/clients error");
...
app.get("/api/clients/all", publicLimiter, async (req, res) => {
    try {
        ...
        logger.error({ err }, "ц?ще▓ GET /api/clients/all error");
...
app.get("/api/admin/clients", publicLimiter, async (req, res) => {
    try {
        ...
        logger.error({ err }, "ц?ще▓ GET /api/admin/clients error");
...
// ----------------------app.put("/api/clients/:id/status/:newStatus", publicLimiter, async (req, res) => {
    try {
        ...
        logger.error({ err }, "ц?ще▓ PUT /api/clients/:id/status/:newStatus error");
...
app.delete("/api/clients/:id", publicLimiter, async (req, res) => {
    try {
        ...
        logger.error({ err }, "ц?ще▓ DELETE /api/clients/:id error");
...
app.post("/api/clients/:id/pdf", publicLimiter, async (req, res) => {
    try {
        ...
        logger.error({ err }, "ц?ще▓ POST /api/clients/:id/pdf error");
...
// 5. Send Client Email (Uses SMTP details)
        ...
        // ц?е⌠Б? REQUIRED SERVER RESPONSE CONTRACT (CORRECTED)
        ...
        logger.error({ err }, "ц?ще▓ POST /api/clients/:id/vcard error");
...
app.listen(PORT, HOST, () => {
    ...
    logger.info(`ц?е?е?Б┌? Server live and listening on ${PUBLIC_URL}`); 
    logger.info(`ц?е?е▓п Frontend expects CORS from: ${ALLOWED_ORIGINS_LOG}`);
});
